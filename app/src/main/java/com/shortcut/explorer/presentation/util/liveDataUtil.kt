...
/**
 * @see mergeLiveData but makes sure that values are nunNull.
 *
 */
fun <E, T,R> LiveData<T?>.mergeNotNull(viewLifecycleOwner: LifecycleOwner, source: LiveData<E?>, block: (T, E) -> R): MediatorLiveData<R> {
    val result = MediatorLiveData<R>()
    result.addSource(source) {
        it?.let { sourceValue->
            this.value?.let { originValue->
